  ...
  try {
    ...
    app.post("/assign-rights", async (req, res) => {
      ...
      try {
        ...
        const transactionHash = await blockchainService.assignRights(
          addressRight,
          percentageOfRights
        );
        ...
        res
          .status(500)
          .send(`Error assigning music rights: ${error.message}`);
      }
    });
    ...
    app.post("/withdraw-rights", async (req, res) => {
      ...
      try {
        ...
        const transactionHash = await blockchainService.withdrawRights(
          addressRight,
          percentageOfRights
        );
        res.send(
          `Music rights withdraw! Transaction hash: ${transactionHash}`
        );
      } catch (error) {
        console.error(error);
        res
          .status(500)
          .send(`Error withdraw music rights: ${error.message}`);
      }
    });
    ...
    app.post("/seal-music-contract", async (req, res) => {
      try {
        ...
        res.status(200).send({
          message: "Music contract sealed successfully!",
          transactionHash,
        });
      } catch (error) {
        console.error("Error sealing music contract:", error);
        res.status(500).send({
          error: `Failed to seal music contract: ${error.message}`,
        });
      }
    });
    ...
    app.post("/buy-rights-music", async (req, res) => {
      try {
        ...
        res
          .status(500)
          .send(`Error purchasing music rights: ${error.message}`);
      }
    });
    ...
    app.post("/listen-music", async (req, res) => {
      try {
        ...
        res.send(`Music listened! Transaction hash: ${transactionHash}`);
      } catch (error) {
        console.error(error);
    ...
    app.get("/remaining-rights", async (req, res) => {
      try {
        const remainingRights =
          await blockchainService.getRemainingRightsDivision();
    ...
    app.get("/musics/producer", async (req, res) => {
      const producerAddress = req.query.producer;
      try {
        const musics = await blockchainService.getMusicsByProducer(
          producerAddress
        );
        res.json(musics);
      } catch (error) {
        console.error(
          `Error fetching musics by producer ${producerAddress}:`,
          error
        );
        res.status(500).send(
          `Error fetching musics by producer ${producerAddress}: ${error.message}`
        );
      }
    });
    ...
    app.get("/music/:id", async (req, res) => {
        ...
        try {
          ...
          if (music) {
            ...
            res.status(404).send(`Music with ID ${id} not found.`);
          }
        } catch (error) {
          console.error(`Error fetching music with ID ${id}:`, error);
          res
            .status(500)
            .send(`Error fetching music with ID ${id}: ${error.message}`);
        }
      });
    ...
    app.post("/music", async (req, res) => {
      ...
      try {
        ...
        res
          .status(201)
          .send({ message: "Music created successfully!", transactionHash });
      } catch (error) {
